...
class SimpleServer(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Radar Club Sahm Is Active and Live!")
...
def get_real_market_data(ticker):
    """جلب الأسعار الحقيقية لمنع تعليق أو تكرار الأسعار الثابتة المخبطة"""
    try:
...
def calculate_time_target(change_val, now_sa):
    """حساب المدى الزمني المتوقع للهدف بدقة وبناءً على عزم حركة السهم الحالية لايف"""
    if change_val > 3.5:
...
def analyze_market_and_send():
    ...
    while True:
        ...
        # 🛑 1. فلتر الإجازة الرسمية (السبت والأحد يقفل الرادار تماماً)
        if current_weekday == 5 or current_weekday == 6:
            print(f"💤 الويكند بدأ.. الرادار مريح الحين يا عبادي.")
            time.sleep(3600)
            continue

        # 🛑 2. فلتر وقت السوق بتوقيت السعودية (4:30 عصراً إلى 11:00 مساءً)
        if current_time_float < 16.5 or current_time_float > 23.0:
            time.sleep(60)
            continue

        # --- مراقبة وتتبع صفقاتك المفتوحة لايف للأهداف والوقف ---
            ...
            if market.get("success"):
                ...
                if live_price >= trade["target1"]:
                    ...
                    msg = (
                        ...
                        f"🔥 *السعر الحالي المكتсح:* ${live_price} (ضرب الهدف الموعود)\n"
                        f"💰 جني أرباح فوري ونقش الغنايم يا عبادي في محفظتك على تطبيق سهم!"
                    )
                    ...
                    msg = (
                        ...
                        f"⚠️ ضرب وقف الخسارة (${trade['stop_loss']}) لسلامة المحفظة."
                    )
                    send_telegram_message(msg)
                    active_trades.pop(ticker)

        # --- فحص الأسواق واقتناص فرص الصعود الحقيقية ---
        for ticker in WATCHLIST:
            ...
            if market.get("success"):
                ...
                if change_val > 1.5:
                    ...
                    # أهداف محسوبة بالملي بناءً على السعر الفعلي المباشر للسهم الحين
                    ...
                    msg = (
                        ...
                        f"───────────────────\n"
                        f"🎯 *المستهدفات (دعم ومقومات الشارت الحقيقية):*\n"
                        ...
                        f"💬 {arabic_news}\n"
                        f"───────────────────\n"
                        f"⚠️ *تنبيه عبادي:* التزم بالسترايك القريب لحماية كاشك وتداولاتك بروقان!"
                    )
                    send_telegram_message(msg)
                    time.sleep(2)

        # التقرير الختامي الساعة 11 بالليل بتوقيت السعودية
...
if __name__ == "__main__":
    ...
    try:
        startup_msg = (
            "🦅 *تم تدشين رادار كلوب الجديد بنجاح يا عبادي!*\n"
            "🎯 البوت الحين مربوط بالتوكن المطور وصار يفحص لايف ويعطيك المدى المتوقع وتاريخ الصيدة بالملي!"
        )
